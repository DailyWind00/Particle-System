...
__kernel void update(__global Particle* particles, int particleCount, float time, float2 window) {
    int id = get_global_id(0); // Get the unique thread ID
    Particle p = particles[id];
    uint seed = id + (uint)(time * 1000.0f) + (uint)(window.x * 1000.0f) + (uint)(window.y * 1000.0f);

    if (p.life <= 0.0f) { // Respawn the particle
        p.position = (float3)(randRange(seed, -1.0f, 1.0f), 1.0f, 0.0f);
        p.velocity = (float3)(0.0f, randRange(seed, -0.01f, -0.025f), 0.0f);
        p.life = 1.0f;
    }
